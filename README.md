...
3. 准备自己的 9288S `kernel.bin`，放入 `runtime\kernel.bin`。
4. 安装 Python 3.11 或更高版本，并安装 NAND 管理依赖：

   ```powershell
   python -m pip install -r requirements-bbk9288s.txt
   ```

5. 双击 `run-bbk9288s-web.cmd`，或在 PowerShell 中运行：
...
Release 不包含 `kernel.bin`。测试 NAND 也作为独立资产发布，不进入 Git
源码历史。请只使用你有权使用和分发的固件与系统文件。
...
Web 前端会延长短触摸和短按键，使原固件的硬件去抖逻辑能够采样。
