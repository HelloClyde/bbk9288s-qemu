...
- 浏览器瞬时触摸会保持 180 ms，瞬时按键会保持 40 ms 后再发 RFB
  release，使原固件去抖有足够采样时间，同时避免一次按键跨越多个
  Timer2-B 系统节拍；事件仍进入 QEMU 的 FPT6、串行触摸 ADC、K5/P0
  GPIO 模型，没有 guest 内存注入、系统 API hook 或应用级快捷通道。
...
- QEMU 新增板级音频解码器总线模型和 `audio-stream` 机器属性。模型只在
  ...
  函数，也没有系统或应用 hook。
- Web 服务从 QEMU 的硬件输出偏移持续等待新码流，检测到一次 guest 播放
  结束后关闭该片段；浏览器按用户手势解码 MP3，并提供扬声器静音按钮和
  音量滑杆。下一次连接从当前硬件字节偏移开始，不会重复旧片段。
