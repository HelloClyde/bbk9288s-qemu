...
- 实现 16-bit Timer2-B 系统节拍：固件从 OSC1 32768 Hz 选择 `/1`，设置 compare B=`292`，模型按约 8.9 ms 周期置 `FIR3 bit2`、复位计数器并投递 vector `38`。固件 ISR 清因子并 post `event1/0x100`，GUI tick 每次增加 10，应用 `SetTimer(hwnd, 1, 50)` 因而能收到 `MSG_TIMER=0x0144`。
...
- 浏览器瞬时 tap 会在前端保持 180 ms 后再发 RFB release，使原固件去抖
  有足够采样时间；事件仍进入 QEMU 的 FPT6、串行触摸 ADC、K5/P0 GPIO
  模型，没有 guest 内存注入、系统 API hook 或应用级快捷通道。
